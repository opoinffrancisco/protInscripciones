...
router.post(`/`,  async (req, res) => {
	...
	// Gestiones
	const resCount = await TIPOPERSONA.getCountAll(conexion_db, req.body);
	if (resCount===false) return res.status(400).json({ error: true, mensajes: `${M.txtGenericos.errorContarRegistros}` });

	const resultados = await TIPOPERSONA.getAll(conexion_db, {por_pagina: req.body.por_pagina, siguiente: (req.body.pagina - 1) * req.body.por_pagina} );
...
router.get(`/:id`,  async (req, res) => {
	...
	// Gestiones
	const busqueda = await TIPOPERSONA.get(conexion_db, { id: req.params.id });
...
router.post(`/registrar`,   async (req, res) => {
	...
	// Validaciones
	const { error } = V.schemaTipoPersona.validate(req.body);
	if (error) return res.status(400).json({  error: true, mensajes: error.details[0].message })
	...
	// Gestiones
	const nuevoRegistro = await TIPOPERSONA.crear(conexion_db, req.body, req.usuario);
...
router.put(`/editar`,  async (req, res) => {
	...
	// Gestiones
	const registroEditado = await TIPOPERSONA.editar(conexion_db, req.body);
...
router.put(`/borrado-logico`,  async (req, res) => {
	...
	// Gestiones
	const eliminacion = await TIPOPERSONA.borradoLogico(conexion_db, { id: req.body.id });
...
router.post(`/borrado-permanente`,  async (req, res) => {
	...
	// Gestiones
	const eliminacion = await TIPOPERSONA.borradoPermanente(conexion_db, { id: req.body.id });
	if (eliminacion===false) return res.status(400).json({ error: true, mensajes: `${M.txtGenericos.errorBorradoPermanentemente} ${txtEntidad}` });
