 ...
 MODELO.getCountAll = async function(connection, datos)
 {
     return new Promise((resolve, reject) => {
         ...
         try {
             ...
             var sql = `SELECT COUNT(*) as total FROM tipo_persona WHERE eliminado=0 ORDER BY id desc LIMIT 1 OFFSET 0`;
             connection.query(
                 {
                     sql: sql, 
                     timeout: timeout_
...
MODELO.getAll = async function(connection, datos)
{
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            var sql = `SELECT * FROM tipo_persona WHERE eliminado=0 ORDER BY id desc LIMIT ${por_pagina} OFFSET ${siguiente}`;
            connection.query(
                {
                    sql: sql, 
                    timeout: timeout_
...
MODELO.get = async function(connection, datos)
{
    return new Promise((resolve, reject) => {
        try {
            ...
            var sql = `SELECT * FROM tipo_persona WHERE id=? AND eliminado=0`;
            connection.query(
                {
                    sql: sql, 
                    values: [datos.id],
...
{    
    return new Promise((resolve, reject) => {
        try {
            var sql = ` INSERT INTO tipo_persona SET ? `;
            connection.query(
                sql, 
                { 
                    nombre:datos.nombre, 
...
{     
    return new Promise((resolve, reject) => {
        try {
            ...
            var sql =   `UPDATE tipo_persona SET 
                            nombre=?,
                            descripcion=? 
                        WHERE id=?`;
            connection.query(
...
{     
    return new Promise((resolve, reject) => {
        try {
            ...
            var sql = `UPDATE tipo_persona SET eliminado=1 WHERE id=?`;
            connection.query(
                sql, 
                [
                    datos.id
...
{     
    return new Promise((resolve, reject) => {
        try {
            ...
            var sql = `DELETE FROM tipo_persona WHERE id =`+ datos.id;
            connection.query(
                sql, 
                async function (error, resultado) {
                    if (error) {
